...
public interface ExpenseRepository extends JpaRepository<ExpenseModel, Integer> {
    ...
    @Modifying
    @Transactional
    @Query("Delete from ExpenseModel e where e.userId=:userId and e.category=:category")
...
//            "FROM ExpenseModel e WHERE e.userId = :userId AND e.date BETWEEN :startDate AND :endDate " +
//            "GROUP BY e.category")
//    public List<ExpenseDto> findTotalExpensesByCategoryAndPeriod(@Param("userId") int userId,
